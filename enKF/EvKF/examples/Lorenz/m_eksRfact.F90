module m_eksRfact
contains
subroutine eksRfact(S,RR,bb,nsing,obsvar)
   use mod_dimensions
   real, intent(in)        :: S(nrtobs,nrsamp)
   real, intent(out)       :: RR(nrtobs,nrtobs)
   real, intent(inout)     :: bb(nrtobs,0:nrsamp)
   ...
   allocate (ipvt(nrtobs))
   allocate (ss(nrtobs))
   allocate (H(nrobs,3))
   allocate (RT(nrtobs*nrtobs))
   allocate (ZZ(nrtobs,nrtobs))
   allocate (w(8*nrtobs))
...
!  Calculating the Eigendecomposition of RR using eispack */
   call rsm(nrtobs,nrtobs,RR,ss,nrtobs,ZZ,w,ipvt,info)
   print *,'rsm ok'
   RR=ZZ
...
!      enddo
!      enddo
!      call sspev(1,RT,ss,RR,nrtobs,nrtobs,w,3)
